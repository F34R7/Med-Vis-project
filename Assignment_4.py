...
if missing_files:
    st.error("Some required CSV files are missing. Make sure the data folder is in the same folder as this script.")
...
cost_relevant = cost_raw[cost_raw["MeasureDesc"] == "Cigarette Sales"].copy()
...
# ## 8. Clean average income data
income_raw.columns = income_raw.columns.str.strip()
year_cols = ["2005", "2006", "2007", "2008", "2009"]
income_relevant = income_raw[["State", "2005", "2006", "2007", "2008", "2009"]].copy()
...
final = pd.merge(final, cancer, on="LocationDesc", how="inner")
final = pd.merge(final, cost_avg, on="LocationDesc", how="inner")
final = pd.merge(final, income_avg, on="LocationDesc", how="inner")
...
st.markdown(
    ...
    **Metric definitions:**
    ...
    - **Cost & Income:** related financial variables.
    - **Ratio metrics:** calculated values used to compare revenue, cost and mortality across states.
    """
)
...
METRIC_MAPPING = {
    ...
    "Income": "Mean_Income",
    "Packs per average Income": "Cost_Relative_To_Income",
...
DISPLAY_LABELS = {
    ...
    "Mean_Income": "Income",
    "Cost_Relative_To_Income": "Packs per avg Income",
...
# ## 11. Organized visualizations
tab_map, tab_bar, tab_heatmap, tab_time, tab_data = st.tabs(
    [
        "🗺️ Map & Scatter Plot",
        "📊 Bar Chart",
...
with tab_map:
    ...
    if map_event and "selection" in map_event and "points" in map_event["selection"]:
        ...
        if state_changed:
            ...
            st.session_state["multiselect_states"] = new_list  
            st.session_state["map_key"] += 1                   
            st.rerun()
...
with tab_time:
    ...
    time_series_1 = income_relevant.copy()
    time_series_2 = cost_relevant[['LocationDesc', 'Year', 'Data_Value']].copy()
